...
class USB_FTX232H_FT60X_sync245mode():
    ...
    def send(self, data):
        ...
        if self.device_type == 'FT60X' and (len(data) & 3) != 0 :
            raise Exception('*** For FT60X, data to be send should be a multiple of 4 bytes! But data to be send is %d bytes here.' % len(data) )
    ...
    def recv(self, recv_len):
        ...
        if self.device_type == 'FT60X' :
            ...
            if (recv_len & 3) != 0 :
                raise Exception('*** For FT60X, data to be receive should be a multiple of 4 bytes! But data to be send is %d bytes here.' % recv_len )
        
            for ii in range(recv_len, 0, -self._chunk):
